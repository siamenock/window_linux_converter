...
BigInt modB(BigInt* i1, BigInt* i2) {
	...
	bool flag = true;
	for (int i = i2->msbLoc + 1; i < MAX; i++) { //digit check
		if (i2->val[i] != 0) {
			flag = false;
			break;
		}
	...
	if (flag) {
		uint32_t temp = i2->val[i2->msbLoc];
		while (temp != 1) { //msb digit check
			if (temp % 2) {
				flag = false;
				break;
			}
		...
		if (flag) { //all check pass
			BigInt ret;
			int loc = i2->msbLoc;
			ret.msbLoc = loc;
			ret.sign = i2->sign;
			ret.val[loc++] = i1->val[loc] % i2->val[loc];
			if (i2->val[loc] == 1)
				ret.msbLoc++;
			while (loc < MAX) {
				ret.val[loc] = i1->val[loc];
				loc++;
			}
			while (ret.val[ret.msbLoc] == 0 && ret.msbLoc != MAX - 1)
				ret.msbLoc++;
